...
#define isOn(S, j) (S & (1 << j)) // check if a particular bit (j-th) is on
#define setBit(S, j) (S |= (1 << j)) // turn the bit on the j-th index 
#define clearBit(S, j) (S &= ~(1 << j)) // clear the bit off in the j-th index
#define toggleBit(S, j) (S ^= (1 << j)) // toggle the bit in the j-th index
#define lowBit(S) (S & (-S))// get the least significant digit
#define setAll(S, n) (S = (1 << n) - 1) // turn on all the bits in a set of size n
...
int main(void) {   
    ...
    S = 34; printSet(S);
    S = S << 1; printSet(S); // mult by 2
    S = S >> 2; printSet(S); // divide by 4
    S = S >> 1; printSet(S); // mult by 2
    printf("\n");
